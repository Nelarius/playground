/* Unify is a C++11 function to normalize resouce identificators.
   ...
   // If `tags` is valid pointer, fill vector with all tags found while parsing.
...
struct disk {
    ...
    std::string operator[]( const std::string &uid_or_uri ) const {
        return lookup( uid_or_uri );
    };
};
// }
...
int main(int argc, const char **argv) {
    ...
    // UID stands for Asset Resouce Identifier.
    ...
    // demo: insertion & lookup from a virtual filesystem
    {
        ...
        #else
        // let's assume following files were inserted: {
        ...
        // }
        #endif
        assert( d["local/file"] == "./local/file.txt" );
        assert( d["local-file"] == "./local/file.txt" );
        assert( d["file-local"] == "./local/file.txt" );
        assert( d["../file/local"] == "./local/file.txt" );
        assert( d["game/icon"] == "./data/game/icon.png" );
        assert( d["game-icon"] == "./data/game/icon.png" );
        assert( d["icon/game"] == "./data/game/icon.png" );
        assert( d["icon-game"] == "./data/game/icon.png" );
        assert( d["songs/main-theme"] == "./songs/main_theme.ogg" );
        assert( d["inventory-json"] == "./game.zip/json #win32/inventory.json" );
        assert( d["logos-big"] == "./game.zip/logos #win32/big.webp" );
        assert( d["logos-big"] == "./game.zip/logos #win32/big.webp" );
    }
